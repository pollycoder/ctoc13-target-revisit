...
{
public:
	int action_;		//标志位，‘0’表示轨道参数初始行，‘1’表示施加脉冲，‘2’表示观测，‘3’表示与中继星开始通讯
	double time_acc_;   //可见时刻
	double rv_acc_[6];  //可见时刻位置速度
	double dv_[3];		//速度增量
	double m_;			//卫星质量，若施加了速度增量，则填施加速度增量后的质量
	int point_id_;		//若action_=0或1，则0占位，若动作标志位为2，则为观测对应的目标编号1~205，若标志位为3，则该列填与本次通信对应的中继星编号1001~1004
	int satnum_;        //1为光学 2为红外
	OutputResult()
	{
		action_ = 0;
		time_acc_ = 0.0;
	...
	OutputResult(int _id, double time, double* rv, double m, int satnum)
	{
		action_ = 2;
		time_acc_ = time;
		for (int i = 0; i < 3; i++)	dv_[i] = 0.0;
	...
	OutputResult(double time, double* RV, double* dv, double m, int satnum)
	{
		action_ = 1;
		time_acc_ = time;
