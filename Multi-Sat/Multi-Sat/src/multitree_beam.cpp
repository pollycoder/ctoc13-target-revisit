...
void MultiTree::Initialize(std::vector<TNC>& expandinglist)
{
	...
	for (int i= 0; i< TreeNum; i++)
	{
		...
		temp_output.satnum_ = i;
		temp_output.m_ = 1000.0;
		temp_output.rv_acc_;
		temp_output.action_ = 0;
		memcpy(temp_output.rv_acc_, sats_rv0[i], 6 * sizeof(double));

		multi_tree_[i].root_->problem_.node_info_.push_back(temp_output);
	}

	//最后将tnc完成
	vector<Node*> old_expand_nodes;
	for (int i=0; i< TreeNum; i++)
	{
...
****************************************************************************/
std::vector<Node*> Tree::ExpandNode(Node* node, const int* visited, const vector<Node_problem> & problem_) //基于问题扩展子节点,本棵树中已经重复的节点就不需要重复了
{
...
****************************************************************************/
inline  void children_nodes(Node* node, const int* visited, vector<Node_problem>& child_node_problems)
{
	...
	for (int j = 0; j < CityNum; j++)
	{
		...
		double mass0, t0, rv0[6], lambda0, phi0, mf, tf, dv[3];
		int  Day, flag0, NR;
		...
		single_imp(mass0, t0, rv0, lambda0, phi0, Day, flag0, mf, tf, dv, NR);// 解析估计
		if (flag0 == 0 || V_Norm2(dv, 3) > 0.5) {
			continue;
		}

		double tend = t0 + tf;
		double mass = mass0;
		int flag1 = solve_trans_dv(city_info[j][0], city_info[j][1], rv0, mass, dv, t0, tend);// 打靶求解
		if (flag1 == 0 || V_Norm2(dv, 3) > 0.5) {
			continue;
		}

		double mass_end, rvf[6];
		
		acc_validate_total(city_info[j][0], city_info[j][1], rv0, mass0, dv, t0, tend, rvf ,mass_end);
		...
		temp_out.satnum_ = node->problem_.node_info_.back().satnum_;


		temp_out2.action_ = 2;
		temp_out2.time_acc_ = tend;
		...
		temp_out2.satnum_ = node->problem_.node_info_.back().satnum_;
...
void MultiTree::Expansion_one_TNC(const TNC& tnc, std::vector<TNC>& newTNCs)
{
	...
	int visited[CityNum]{};                     //该tnc的已观测序列，初始化为0，观测后为1
	...
	for (int i = 0; i < TreeNum; i++) //按树扩展
	{
		...
		vector<Node_problem> child_node_problems;
		children_nodes(tnc.tnc_[i], visited, child_node_problems);     //排除掉那些很大速度增量的碎片 TODO: 需要改

		omp_set_lock(&tnc.tnc_[i]->lock);
		vector<Node*> leafnode = multi_tree_[i].ExpandNode(tnc.tnc_[i], visited, child_node_problems); //该节点扩展的子节点
...
void MultiTree::Run()
{
	...
	while (expandinglist.size() > 0 && layer_ < CityNum) //非空
	{
...
void MultiTree::RecordBestResult(std::vector<TNC>& expandinglist, std::ofstream& fout1)
{
	...
	std::cout << "Removal total num, Total mass, Time, Height:  " << result_now_.total_observed_num_ << " " << result_now_.total_mass_ << " " << result_now_.time_cost_ << " " << result_now_.height_aver_<< std::endl;
	fout1 << "Removal total num, Total mass, Time, Height:  " << result_now_.total_observed_num_ << " " << result_now_.total_mass_ << " " << result_now_.time_cost_ << " " << result_now_.height_aver_ << std::endl;
	...
	for (int id_sat = 0; id_sat < TreeNum; id_sat++)
	{
		...
		{
			fout1 << std::fixed << i + 1 << " " << result_now_.solution_[id_sat].node_info_.back().satnum_ << " "
